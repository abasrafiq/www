...
class MySQLDump extends CI_Model {
	/**
	* @access private
	*/
  ...
  /**
	...
	*/
	function MYSQLDump($db = null, $filepath = '', $compress = false, $hexValue = false){
	...
	*/
    ...
    function setDumpTables($tables)
    {
        if(is_array($tables)){$this->dTables=  $tables; }
	...
	*/
	function getDatabaseData($hexValue = true)
    {
		if(!$this->dTables){
            ...
            foreach ($this->dTables as $table){                
                $this->getTableData($table,$hexValue);
            }
